...
def build_topk_dict(
    sim_pairs: List[Tuple[int, int, float]], n_users: int, topk: int
    ...
    topk_dict = {
        node: sorted(neighbors, key=lambda x: x[1], reverse=True)[:topk]
        for node, neighbors in sim_dict.items()
    }

    return {
        str(node): [{"node": int(nei), "sim": float(sim)} for nei, sim in neighbors]
        for node, neighbors in topk_dict.items()
    }


def main():
    parser = argparse.ArgumentParser(description="用户嵌入相似度计算 JSON 输出")
    parser.add_argument(
    ...
    parser.add_argument(
        "--topk", type=int, default=10, help="每个节点保留的 topk 相似节点数"
    )
    parser.add_argument(
        "--metric",
        type=str,
    ...
    topk_result = build_topk_dict(sim_pairs, emb.shape[0], args.topk)

    output_str = json.dumps(topk_result, indent=2)
